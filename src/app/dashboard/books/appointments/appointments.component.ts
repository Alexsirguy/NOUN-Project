...
export class AppointmentsComponent extends ChangeDetectRefComponent implements OnInit {
  ...
  constructor(public media: MediaMatcher, private router: Router, private authService: AuthService, public changeDetectorRef: ChangeDetectorRef, private uiService: UiService, private httpClient: HttpRequest, private title: DataService, private dialog: MatDialog,
    ...
    getTransaction() {
      this.onLoading()
      this.httpClient.get(`users/appointments`).then(response => {
          if (response.code === 1) {
    ...
    appointment(id){
      this.uiService.openDialogV2(BookDetailsDialogComponent, this.changeDetectorRef, {id:id }, (data) => { });
    }
  }
